...
parkingRouter.get("/", async (req, res) => {
    try {
        ...
        const parkingWithAdminRatings = parking.map((item) => {
            let rating = 0;
            let count = 0;
            const userReviews = reviews.filter((review) => {
             console.log('review ', review);
             console.log('item?.user_id ', item?.user_id);
             return   review.admin_id.equals(item?.user_id?._id)
            })
            ...
            admin_rating = rating > 0 ? (rating / count) : 0;

            return { ...item.toObject(), admin_rating };
        })
       
        res.json(parkingWithAdminRatings);
    } catch (error) {
        console.error('error ', error);
        res.status(400).json({ error });
    }
